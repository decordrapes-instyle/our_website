...
const QuotationModal = ({ quotation, onClose }: { quotation: any; onClose: () => void }) => {
    ...
    useEffect(() => {
        ...
        const handleTouchMove = (e: TouchEvent) => {
            ...
            if (touchCurrentY > touchStartY.current) {
                e.preventDefault();
            }
        };
    ...
    return (
        <div 
            className={`fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-end md:items-center ${isClosing ? 'animate-fadeOut' : 'animate-fadeIn'}`}
            onAnimationEnd={handleAnimationEnd}
            onClick={handleOverlayClick}
        >
            <div 
                className={`bg-white dark:bg-neutral-900 rounded-t-2xl md:rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col transform transition-transform duration-300 ${isClosing ? 'animate-slide-out-down' : 'animate-slide-in-up'}`}
                onClick={(e) => e.stopPropagation()}
            >
                <div 
                    ref={headerRef}
                    className="flex-shrink-0 p-6 pt-4 border-b border-gray-200 dark:border-neutral-700"
                    onTouchStart={handleTouchStart}
                    onTouchEnd={handleTouchEnd}
                >
                    <div className="w-12 h-1.5 bg-gray-300 dark:bg-neutral-700 rounded-full mx-auto mb-4 cursor-grab md:hidden"></div>
                    <div className="flex justify-between items-center">
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Quotation Details</h2>
                        <button onClick={handleClose} className="text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 p-2 rounded-full bg-gray-100 dark:bg-neutral-800">
                            <X size={20} />
                        </button>
                    </div>
                </div>
